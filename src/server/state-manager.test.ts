...
describe('stamps are always leaves', () => {
  it('adds a child asked for under a stamp as the stamp\'s sibling', () => {
    const { sm } = harness()
    const title = sm.createTitle(ROOT_NODE_ID, 0, 0, 'section')
    const stamp = sm.createStamp(title.id, 10, 10, 'star')
    expect(stamp.parentId).toBe(title.id)

    expect(sm.createTitle(stamp.id, 0, 0, 'x').parentId).toBe(title.id)
    expect(sm.createMarkdown(stamp.id, 0, 0).parentId).toBe(title.id)
    expect(sm.createStamp(stamp.id, 0, 0, 'double-exclamation').parentId).toBe(title.id)
    expect(createTerminal(sm, 'under-stamp', stamp.id).parentId).toBe(title.id)
  })

  it('refuses to reparent anything onto a stamp', () => {
    const { sm } = harness()
    const stamp = sm.createStamp(ROOT_NODE_ID, 0, 0, 'star')
    const title = sm.createTitle(ROOT_NODE_ID, 0, 0, 'x')
    sm.reparentNode(title.id, stamp.id)
    expect(sm.getNode(title.id)?.parentId).toBe(ROOT_NODE_ID)
  })

  it('refuses a parent/child swap that would make a stamp the parent', () => {
    const { sm } = harness()
    const title = sm.createTitle(ROOT_NODE_ID, 0, 0, 'x')
    const stamp = sm.createStamp(title.id, 0, 0, 'star')
    sm.swapParentChild(title.id, stamp.id)
    expect(sm.getNode(title.id)?.parentId).toBe(ROOT_NODE_ID)
    expect(sm.getNode(stamp.id)?.parentId).toBe(title.id)
  })

  it('can itself be reparented', () => {
    const { sm } = harness()
    const title = sm.createTitle(ROOT_NODE_ID, 0, 0, 'x')
    const stamp = sm.createStamp(ROOT_NODE_ID, 0, 0, 'star')
    sm.reparentNode(stamp.id, title.id)
    expect(sm.getNode(stamp.id)?.parentId).toBe(title.id)
  })
})
