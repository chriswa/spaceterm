...
export function App() {
  ...
  const createChildNode = useCallback(async (parentNodeId: NodeId, type: AddNodeType, hint?: { x: number; y: number }): Promise<NodeId> => {
    ...
    switch (type) {
      ...
      // Generated cards are materialised by the server from what is on disk.
      // They are absent from the add menu, so this is unreachable through the
      // UI — it exists because the switch is over every CardType, and a silent
      // fallthrough here would return an unassigned id to `navigateToNode`.
      case 'meta-group':
      case 'meta-doc':
        throw new Error(`${type} cards are generated by the agent-meta branch, not added by hand`)
    }
    if (cwd) cwdMapRef.current.set(nodeId, cwd)
