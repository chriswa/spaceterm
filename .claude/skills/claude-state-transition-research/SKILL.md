...
`PreToolUse`/`PostToolUse`/`PreCompact` from subagents fire on the same surface ID as the main agent, but they carry an `agent_id` in the payload. `handleHook` ignores `PreToolUse`/`PreCompact` that carry an `agent_id` (subagent background work) so they can't flip `working_background` (yellow) back to `working` while the main agent is idle, and can't corrupt main-agent permission correlation (lastPreToolUseId). `PostToolUse` is additionally protected by tool_use_id matching, and the sticky-waiting-states guard still protects waiting states. Subagent JSONL never reaches the main transcript (sidechain entries live in `<sid>/subagents/agent-<id>.jsonl`, verified 0/300 main transcripts), so JSONL working-signals need no equivalent guard. Only `SubagentStart`/`SubagentStop` consume `agent_id` (to drive the background ledger).
